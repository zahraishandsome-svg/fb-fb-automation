#!/usr/bin/env python3
"""
Refresh the SOURCE Facebook Page access token.

Usage:
  python refresh_source_token.py --page page_1 --token "EAAxxxxx..."
...
Steps:
  ...
  4. Saves to tokens/page_N_source_token.json with expiry date
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Refresh source Facebook Page access token")
    ...
    print(f"Loading source credentials from: {channel['source_credentials_file']}")
    creds = load_credentials(channel["source_credentials_file"])
    app_id = creds["app_id"]
    ...
    print(f"Fetching Page access token for source page_id={page_id}...")
    ...
    save_token(channel["source_token_file"], token_data)
    print(f"Saved to: {channel['source_token_file']}")
