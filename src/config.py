...
def _validate_channel(ch: Dict[str, Any]) -> Dict[str, Any]:
    required = [
        "id", "source_page_id", "source_page_name", "dest_page_name",
        "source_credentials_file", "source_token_file",
        "dest_credentials_file", "dest_token_file",
    ]
    for field in required:
        if not ch.get(field):
            raise ValueError(f"Channel config missing required field: '{field}'")

    ch["source_credentials_file"] = PROJECT_ROOT / ch["source_credentials_file"]
    ch["source_token_file"] = PROJECT_ROOT / ch["source_token_file"]
