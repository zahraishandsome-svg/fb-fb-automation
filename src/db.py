    ...
    with conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS posted_videos (
                ...
                created_at          TEXT DEFAULT CURRENT_TIMESTAMP,
                updated_at          TEXT DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY (channel_id, source_video_id)
            );
...
def upsert_channel(channel_cfg: Dict[str, Any]) -> None:
    ...
    with conn:
        conn.execute("""
            INSERT INTO channels (id, source_page_name, dest_page_name, enabled, updated_at)
            VALUES (:id, :source_page_name, :dest_page_name, :enabled, :now)
            ON CONFLICT(id) DO UPDATE SET
                source_page_name = excluded.source_page_name,
                dest_page_name   = excluded.dest_page_name,
                enabled          = excluded.enabled,
                updated_at       = excluded.updated_at
        """, {
            "id": channel_cfg["id"],
            "source_page_name": channel_cfg["source_page_name"],
            "dest_page_name": channel_cfg["dest_page_name"],
...
def get_posted_video_ids(channel_id: str) -> set:
    conn = get_connection()
    rows = conn.execute("""
        SELECT source_video_id FROM posted_videos
        WHERE channel_id = ? AND status IN ('uploaded', 'failed_permanent', 'skipped')
    """, (channel_id,)).fetchall()
    conn.close()
    return {row["source_video_id"] for row in rows}
...
def mark_uploaded(channel_id: str, source_video_id: str, dest_video_id: str) -> None:
    conn = get_connection()
    with conn:
        conn.execute("""
            ...
            SET status = 'uploaded', dest_video_id = ?, posted_at = ?,
                ...
                updated_at = ?
            WHERE channel_id = ? AND source_video_id = ?
        """, (
            dest_video_id,
    ...
    with conn:
        if current_count > max_retries:
            conn.execute("""
                ...
                WHERE channel_id = ? AND source_video_id = ?
            """, (current_count, error_message, now, channel_id, source_video_id))
            logger.warning("Video %s on channel %s permanently failed after %d retries",
                           source_video_id, channel_id, current_count)
        else:
            conn.execute("""
                UPDATE posted_videos
