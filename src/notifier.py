...
def send_failure_alert(webhook_url: str, failures: List[Dict[str, Any]], slot: int) -> None:
    if not webhook_url:
        return
    lines = [f"🚨 **FB→FB Automation | Slot {slot} Failures** ({date.today()})"]
    for f in failures:
...
def send_daily_summary(
    ...
    lines = [
        f"{emoji} **FB→FB Daily Summary** ({date.today()})",
        f"Uploaded: {len(success_rows)} | Failed: {len(failed_rows)} | No content: {len(no_content_rows)}",
    ]

    for r in success_rows:
        dest_id = r.get("dest_video_id")
        url = f"https://www.facebook.com/video/{dest_id}" if dest_id else "?"
